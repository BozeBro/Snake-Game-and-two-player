...
def game(players=2):
    ...
    def who_ate(snakes):
        # Finds out which snake object will not be popleft
        for user in snakes:
    ...
    def move_snake(user, other_user, loser):
        if not user:
            return True
    ...
    if players == 2:
        ...
        snake2 = []
    snakes = (snake, snake2)
    apple = Apple(snakes, game_values, color=RED)
    apple.make_rect(screen)
    pygame.display.flip()
    ...
    while running and not end:
        ...
        running, winner = move_snake(snake2, snake, "snake") and move_snake(
            snake, snake2, "snake2"
        )
